...
# confusión y curvas PR. Guarda resultados en logs/<var>/test/...
#==============================================================
...
def _images_dir_from_split(ds: dict, split: str) -> Path:
    # dataset.yaml puede tener rutas absolutas por split
    if split in ds and ds[split]:
        return Path(ds[split])
    # o construir a partir de 'path'
...
def _labels_dir_for_images_dir(images_dir: Path) -> Path:
    p = str(images_dir)
    if "images" in images_dir.name:
        return images_dir.with_name("labels")
    # Reemplazo robusto por carpeta
    return Path(p.replace("/images", "/labels").replace("\\images", "\\labels"))
...
def _iou_xyxy(a: np.ndarray, b: np.ndarray) -> float:
    """IoU escalar entre dos cajas XYXY (arrays de 4 floats)."""
...
def _build_pred_from_gt(gt_row: np.ndarray,
                        ...
                        conf: float = 0.9,
                        wrong_class: bool = False) -> np.ndarray:
    """Genera 1 pred a partir de un GT en formato [cls,cx,cy,w,h] (normalizado)."""
    ...
    cls = int(c)
    if wrong_class:
        cls = (cls + 1)  # clase equivocada determinística
    pred = np.array([xyxy[0], xyxy[1], xyxy[2], xyxy[3], conf, float(cls)], dtype=np.float32)
    return pred
...
def _overlay_debug(image_bgr: np.ndarray,
    ...
    if gt_xyxy is not None:
        ...
        cv2.rectangle(im, (x1, y1), (x2, y2), (0, 255, 0), 2)  # GT en verde
    ...
    if pred_xyxy is not None:
        ...
        cv2.rectangle(im, (x1, y1), (x2, y2), (36, 36, 255), 2)  # Pred en rojo
...
def main() -> None:
    ...
    # Nombre de corrida unificado (usado por overlay, figuras y CSV)
    ...
    if len(y) == 0:
        ...
        # Caso negativo: sin GT. Podríamos inyectar FP para probar.
        gt = torch.zeros((0, 6), dtype=torch.float32)
        preds = []
        for _ in range(args.extra_fp):
            # FP aleatorio en alguna clase válida
        ...
        # Tomamos el primer GT (o el de mayor área)
        areas = y[:, 3] * y[:, 4]
        gt_row = y[int(np.argmax(areas))]  # [cls,cx,cy,w,h]
        # GT para métrica (formato [img, cls, cx, cy, w, h])
        ...
        pred_np = _build_pred_from_gt(gt_row, (H, W), jitter=args.jitter, scale_jitter=args.scale_jitter,
                                      conf=args.conf, wrong_class=args.wrong_class)
        preds = [pred_np]
        # FP adicionales si se solicita
        for _ in range(args.extra_fp):
            cx, cy, w, h = [random.random() for _ in range(4)]
            xyxy = _xywhn_to_xyxy_pix(np.array([[cx, cy, w * 0.2, h * 0.2]], dtype=np.float32), (H, W))[0]
            c = float(random.randrange(len(class_names)))
        ...
        # Guardar overlay visual (opcional pero útil)
        ...
        print(f"[Debug] IoU(GT,PRED) = {_iou_xyxy(gt_xyxy, pred_np[:4]):.3f}")
    ...
    # --- Métricas ---
    # run_name definido anteriormente
    ...
    summary, curves = metricor.finalize()

    # --- Logging CSV (directo en save_dir) ---
