...
#--------# AMP migration: torch.cuda.amp → torch.amp to avoid FutureWarning on newer PyTorch versions
# NOTE: We now rely on torch.amp.autocast("cuda", ...) and torch.amp.GradScaler("cuda", ...).
...
def train(opt, device):
    ...
    best_fitness = 0.0
    #--------# AMP migration: use torch.amp.GradScaler instead of torch.cuda.amp.GradScaler
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            # Forward
            #--------# AMP migration: use torch.amp.autocast with explicit device type instead of torch.cuda.amp.autocast
