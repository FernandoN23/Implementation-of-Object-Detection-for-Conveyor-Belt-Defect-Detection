...
# -------- Proyecto AMP-migration: compatibilidad torch.amp / torch.cuda.amp --------
try:  # pragma: no cover - compatible con PyTorch ≥ 2.0 y fallback legacy
    from torch.amp import autocast as _torch_amp_autocast, GradScaler as _TorchGradScaler


    def torch_amp_autocast(enabled: bool = True):
        """Wrapper unificado para autocast que usa torch.amp.autocast en GPU CUDA/HIP.

        En PyTorch ≥ 2.0 se recomienda usar torch.amp.autocast('cuda', enabled=...).
        Este wrapper evita el uso directo de torch.cuda.amp.autocast (deprecado).
        """
        return _torch_amp_autocast(device_type="cuda", enabled=enabled)


    def TorchGradScaler(enabled: bool = True):
        """Wrapper unificado para GradScaler que usa torch.amp.GradScaler.

        Mantiene la firma GradScaler(enabled=...), evitando el uso directo de
        torch.cuda.amp.GradScaler (deprecado) en entornos modernos.
        """
        return _TorchGradScaler(enabled=enabled)

except Exception:  # pragma: no cover - fallback para versiones anteriores de PyTorch
    from torch.cuda.amp import autocast as _torch_amp_autocast, GradScaler as _TorchGradScaler


    def torch_amp_autocast(enabled: bool = True):
        """Fallback a torch.cuda.amp.autocast para versiones antiguas de PyTorch."""
        return _torch_amp_autocast(enabled=enabled)


    def TorchGradScaler(enabled: bool = True):
        """Fallback a torch.cuda.amp.GradScaler para versiones antiguas de PyTorch."""
        return _TorchGradScaler(enabled=enabled)
# ------------------------------------------------------------------------------
import yaml
...
# --------------------------------------------------------------------
# Integración opcional con el parche BN→GN del proyecto externo (YOLO/engine/bn2gn_patch.py).
# Si el módulo no está disponible (por ejemplo, al usar YOLOv5 de forma aislada), estos
...
def train(hyp, opt, device, callbacks):
    ...
    # --------------------------------------------------------------------------------------------------
    # BN→GN integration (opcional, para entornos ROCm/MIOpen)
    # Los parámetros llegan desde el orquestador externo (YOLO/train.py → Trainer → opt.bn2gn_*).
    # Si el módulo de parche no está disponible o la política es "off", no se aplica ningún cambio.
    bn2gn_policy = str(getattr(opt, "bn2gn_policy", "off") or "off").lower()
    if apply_bn2gn_patch is not None and BN2GNConfig is not None and bn2gn_policy in {"on", "on_error"}:
        cfg_bn2gn = BN2GNConfig(
            policy=bn2gn_policy,
            max_groups=int(getattr(opt, "bn2gn_max_groups", 32)),
            min_channels_per_group=int(getattr(opt, "bn2gn_min_channels_per_group", 1)),
            verbose=int(getattr(opt, "bn2gn_verbose", 1)),
        )
        apply_bn2gn_patch(
            model,
        ...
        # -------- Proyecto BN2GN: marca de política aplicada en el modelo --------
    ...
    # -------- Proyecto AMP-migration: usar wrapper TorchGradScaler unificado --------
    scaler = TorchGradScaler(enabled=amp)
    # ---------------------------------------------------------------------------
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    ...
                    # -------- Proyecto BN2GN: evitar fuse() con GroupNorm en validación final --------
                    bn2gn_policy_for_best = bn2gn_policy
                    fuse_for_best = bn2gn_policy_for_best not in {"on", "on_error"}
                    model_for_best = attempt_load(f, device, fuse=fuse_for_best).half()
                    # ---------------------------------------------------------------------------------
