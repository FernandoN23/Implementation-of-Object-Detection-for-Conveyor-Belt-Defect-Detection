...
#              y visualizaciones de predicciones (overlays).
# ==============================================================
...
def _load_module_from_local(path: Path, name: str):
    ...
    # Inyección temporal de path.parent en sys.path (CRÍTICO para ssd/utils/metrics.py)
...
def _process_batch_stats(detections, gt_boxes, gt_labels, iouv):
    ...
    correct = torch.zeros(detections.shape[0], niou, dtype=torch.bool, device=detections.device)
    ...
    if len(gt_labels) > 0:
        ...
        for i, iou_thres in enumerate(iouv):
            ...
            # x: (indices_pred, indices_gt) donde IoU > threshold Y clases coinciden
            x = torch.where((iou_matrix >= iou_thres) & (pred_labels[:, None] == gt_labels[None, :]))

            if x[0].shape[0]:
                # matches: [N_matches, 3] -> (idx_pred, idx_gt, iou_val)
                ...
                if x[0].shape[0] > 1:
                    ...
                    # Eliminar GT duplicados (solo el mejor IoU gana)
                    matches = matches[np.unique(matches[:, 1], return_index=True)[1]]
                    # Eliminar Pred duplicados (solo el mejor IoU gana)
                    matches = matches[np.unique(matches[:, 0], return_index=True)[1]]

                if matches.shape[0] > 0:
                    correct[matches[:, 0].astype(int), i] = True

                    # Solo guardar IoUs para el primer threshold (0.5)
                    if i == 0:
...
class ValidatorSSD:
    ...
    def __init__(
        ...
        # FIX F.1: Acceder a los parámetros de inferencia directamente desde cfg
        ...
        self.iouv = torch.linspace(0.5, 0.95, 10, device=self.device)  # IoU thresholds para mAP
    ...
    def _get_detections_from_output(output: torch.Tensor, img_size: Tuple[int, int], conf_thresh: float,
        ...
        # output: [1, num_classes, top_k, 5] -> (score, x1, y1, x2, y2)
        ...
        for cls_idx in range(1, output.size(1)):
            ...
            # Las coordenadas ya están escaladas por la capa Detect de SSD, pero están normalizadas [0,1]
            # Las re-escalamos a píxeles
            boxes = dets_cls[:, 1:] * scale
            labels = torch.full((dets_cls.size(0), 1), cls_idx - 1, device=device)
    ...
    def calculate_metrics_only(
        ...
        """
        Ejecuta la validación para obtener solo los 5 valores promedio de métricas.
        Usado por el Trainer para el logging histórico.
        """
        model.eval()
        ...
        stats = []
        # FIX F.1: Acceder a conf_thresh directamente desde cfg
        conf_thresh = getattr(cfg, "conf_thresh", 0.01)

        for images, targets in data_loader:
            images = images.to(device)
            output = model(images)  # [batch, num_classes, top_k, 5]

            for i, pred_raw in enumerate(output):
                img_size = images[i].shape[1:]

                # 1. Obtener predicciones en formato [N, 6] (x1, y1, x2, y2, conf, cls)
                ...
                gt_boxes = gt[:, :4] * torch.tensor([img_size[1], img_size[0], img_size[1], img_size[0]], device=device)
                gt_labels = gt[:, 4]
        ...
        if not stats or not stats[0].any():
    ...
    def run_full_report(self, num_images_to_plot: int = 32) -> Dict[str, float]:
        """Ejecuta el ciclo completo de validación, genera gráficos y visualizaciones."""
        self.model.eval()
        ...
        for batch_i, (images, targets) in enumerate(pbar):
            ...
            for i, pred_raw in enumerate(output):
                ...
                # 1. Obtener predicciones en formato [N, 6] (x1, y1, x2, y2, conf, cls)
                # Usamos self.conf_thresh (bajo, ej. 0.001) para generar curvas completas
                pred_tensor = self._get_detections_from_output(
                    pred_raw.unsqueeze(0), img_size, self.conf_thresh, self.device
                )

                # 2. Preparar Ground Truth (escalado a píxeles)
        ...
        # FIX: Extraer AP@0.5 por clase para la impresión
        ap50 = ap[:, 0]
    ...
    def _select_images_for_visualization(self, n: int) -> set:
        """Selecciona 'n' stems de imágenes de forma reproducible (usando seed)."""
    ...
    def _plot_single_overlay(self, img_tensor, pred_tensor, gt_boxes, gt_labels, stem):
        """Dibuja GT y Predicciones sobre una imagen y la guarda."""

        # Convertir tensor BGR (con medias restadas) a imagen RGB (uint8)
        img_np = img_tensor.permute(1, 2, 0).cpu().numpy()
        mean = self.cfg.augmentation.mean
        img_np = img_np + np.array(mean)
        img_np = np.clip(img_np, 0, 255).astype(np.uint8)
        img_bgr = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)  # OpenCV usa BGR

        # Colores (GT: verde, Pred: naranja/amarillo)
        color_gt = (0, 255, 0)
        color_pred = (0, 165, 255)  # Naranja

        # 1. Dibujar Ground Truth (GT)
    ...
    def _plot_results(self, stats, iou_vals, confusion_matrix, map50, map95):
        """Orquesta la generación de todos los gráficos estáticos."""
    ...
    def _plot_curves_yolo_style(self, stats, map50):
        ...
        # P Curve
        best_p_idx = mean_p.argmax()
        best_p = mean_p[best_p_idx]
        best_p_conf = px[best_p_idx]
        ax_p.plot(px, mean_p, linewidth=4, color="blue", label=f"all classes {best_p:.2f} at {best_p_conf:.3f}")
        ...
        # R Curve
        best_r_idx = mean_r.argmax()
        best_r = mean_r[best_r_idx]
        best_r_conf = px[best_r_idx]
        ax_r.plot(px, mean_r, linewidth=4, color="blue", label=f"all classes {best_r:.2f} at {best_r_conf:.3f}")
