...
# Autor: Fernando N.
# --------------------------------------------------------------
# Archivo: DINO/engine/Validator.py
# Descripción: Motor de validación para DINO. Evalúa el modelo
...
class Validator:
    ...
    @torch.no_grad()
    def validate(self, loader, output_dir, model_override=None):
        """Validación rápida para el ciclo de entrenamiento (mAP COCO)."""
        # Permite usar el modelo EMA si se pasa como argumento
        ...
        with MuteStderr():
            for samples, targets in loader:
                ...
                # En inferencia, DINO NO recibe targets (se apaga el Contrastive DeNoising)
    ...
    def _plot_single_overlay(self, img_bgr, pred_boxes, pred_scores, pred_labels, gt_boxes, gt_labels, class_names,
        ...
        for cx, cy in corners:
            overlap_area = 0
            lx1, ly1, lx2, ly2 = cx, cy, cx + lw, cy + lh
